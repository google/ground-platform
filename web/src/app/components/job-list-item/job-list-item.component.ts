...
export class JobListItemComponent implements OnInit, OnDestroy {
  ...
  async deleteJob() {
    await this.dataStoreService.deleteJob(this.projectId!, this.job!.id);
    this.onClose();
  }
  ...
  onImportFeatures() {
    if (!this.projectId || !this.job?.id) {
      return;
    }
    this.importDialog.open(ImportDialogComponent, {
      data: { projectId: this.projectId, jobId: this.job?.id },
      width: '350px',
      maxHeight: '800px',
    });
  }
  ...
  getDownloadCsvUrl() {
    return (
      ...
      `project=${this.projectId}&job=${this.job?.id}`
    );
  }
