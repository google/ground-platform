...
sealed interface XPathValue {
  ...
  companion object {
    ...
    fun formatXPathNumber(value: Double): String {
      ...
      if (value == 0.0) {
        // `-0.0 == 0.0` is true in Kotlin, so compare bit patterns to keep the sign. XPath 1.0
        // section 4.3 requires string(-0) to be "-0".
        return if (value.toRawBits() == (-0.0).toRawBits()) "-0" else "0"
      }
      if (
        value % 1.0 == 0.0 &&
