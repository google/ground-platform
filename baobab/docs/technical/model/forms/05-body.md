...
thresholds, and mock provider settings corresponding to XForms attributes:

```protobuf
message GeoConfig {
  // Target horizontal accuracy threshold in meters for auto-acceptance
  // (XForms `accuracyThreshold` attribute, XLSForm `capture-accuracy` / `body::accuracyThreshold`).
  double accuracy_threshold_meters = 1;

  // Horizontal accuracy threshold in meters above which the UI displays an
  // uncertainty warning (XForms `unacceptableAccuracyThreshold` attribute, XLSForm
...
standard XForms bindings and appearances:
...
*   **Proximity Guardrail**: Configured in `FieldBinding.constraint_expression`
    using the XForms XPath `distance()` function: `distance(.,
