...
onedoc_md_file_id: 17540b83-8070-41cd-806e-b67055dabc9c
onedoc_tab_id: t.9nlxzx2c7bzm
onedoc_tab_title: Future Work
---
...
## Support for Visual Classification

Expanding beyond field-based ground surveys, future iterations will integrate
remote-sensing visual interpretation and augmented sampling workflows directly
within the Ground Web Console:

*   **Sample Design Generation**: Built-in statistical sampling engines allowing
    survey designers to generate probabilistic sampling designs (e.g.,
    systematic grids, stratified random sampling, cluster sampling) bounded by
    user-defined regions of interest (ROIs), administrative boundaries, or
    uploaded shapefiles and GeoJSON features.
*   **Custom Raster Layers**: Support for incorporating custom imagery and
    analytical raster layers beyond standard streaming basemaps, including
    high-resolution drone/UAV orthomosaics, user-supplied Cloud-Optimized
    GeoTIFFs (COGs), custom tile services (XYZ, TMS, WMS), and Earth
    Engine-streamed multi-spectral indices (e.g., NDVI, EVI, SAR backscatter).
*   **Streamlined Web-Based Labelling Workflows**: High-throughput visual
    photo-interpretation interface in the Web Console featuring rapid keyboard
    navigation, sub-plot sampling grids (e.g., 3x3 point grids per plot),
    multi-temporal satellite time-series comparisons (e.g., Planet NICFI monthly
    mosaics and Sentinel-2 seasonal composites), and multi-interpreter consensus
    and QA/QC review tools.
...
3.  **Entity-Linked Follow-Up Submissions (1:N)**: Configuring validation
    surveys as multiple follow-up encounters linked to preloaded alert entities,
    allowing mobile users to tap flagged map markers to launch pre-linked forms
    that dynamically pull in model certainty scores via `pulldata()`.
