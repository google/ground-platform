...
A survey transitions through four lifecycle states:

| State      | Description                                                    |
| ---------- | -------------------------------------------------------------- |
| `DRAFT`    | Staged in draft mode for creation, refinement, and testing     |
:            : without affecting ongoing field operations.                    :
| `ACTIVE`   | Published release active for mobile synchronization and field  |
:            : data collection.                                               :
| `ARCHIVED` | Retired survey no longer active for new field data collection. |
| `DELETED`  | Deleted survey, removed from all user interfaces.              |
...
While ODK XForms (`FormDef`) defines a single static `<h:title>` and has no
