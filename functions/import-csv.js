 ...
 * 'project' id and 'job' id to the database.
 */
async function importCsv(req, res) {
  ...
  busboy.on("file", (key, file, _) => {
    ...
    console.log(
      `Importing CSV into project '${projectId}', job '${jobId}'`
    );
    ...
    file.pipe(csvParser()).on("data", async (row) => {
      try {
        inserts.push(insertRow(projectId, jobId, row));
      } catch (err) {
        console.error(err);
        res.unpipe(busboy);
        await res
...
async function insertRow(projectId, jobId, row) {
